...
for i in range(len(lst_RNA)):
    ...
    N = data.size
    comment1 = 'N = ' + str(N)
    plt.text(20.7, 0.17, comment1, horizontalalignment='right', size=12, fontweight='normal')

    percent = round((data[data>=19].size / N) * 100, 2)
    comment2 = 'Static: ' + str(percent) + '%'
    plt.text(20.7, 0.15, comment2, horizontalalignment='right', size=12, fontweight='heavy')
